...
```{r testm, warning=FALSE, echo=FALSE, message=FALSE}

library(plotly)
p1<-ggplotly(selectionEstimateFits[["Canada"]]$plot1)
p2<-ggplotly(selectionEstimateFits[["Canada"]]$plot2)
...
plot <- subplot(p1, p2, nrows = 1, shareX = T, shareY = T)

# Add the layout to the plot object
plot <- plot %>% layout(
  updatemenus = list(menu)
)
plot
...
plot_dropdown <- function(plot1, plot2) {
  # Convert the ggplot objects to plotly objects
  p1 <- ggplotly(plot1)
  p2 <- ggplotly(plot2)

  # Combine the two plotly objects and add a dropdown menu to switch between them
  ...
  fig <- fig %>% layout(
    updatemenus = list(
      list(
        buttons = list(
          list(method = "update",
               args = list(list(visible = c(TRUE, FALSE)), list(title = "Plot 1"))),
          list(method = "update",
               args = list(list(visible = c(FALSE, TRUE)), list(title = "Plot 2")))
        ),
        direction = "down",
        showactive = TRUE,
        x = 0.1,
...
# Call the plot_dropdown function with the two ggplot objects
plot_dropdown(plot1, plot2)
...
for (i in 1:length(all.regions$name)){
 ...
 #individualSublineageSelectionPlots.BA2[[all.regions$shortname[[1]]]][[1]]$variant
 #view(individualSublineageSelectionPlots.BA2[[all.regions$shortname[[1]]]][[1]]$plot$data)
 #view(individualSublineageSelectionPlots.BA2[[all.regions$shortname[[1]]]][[1]]$plot$line)
