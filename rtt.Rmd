...
get.segments <- function (fit) {
  p <- predict(fit)
  x <- fit$model$x
  list(x0=min(x), x1=max(x), y0=min(p), y1=max(p))
}
...
r2d3(data=toJSON(list(tips=rtt1$tips, palette=as.list(pal),
                      fits=lapply(fit1, get.segments)), auto_unbox=TRUE), 
