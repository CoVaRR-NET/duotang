...
if(!is.null(fit1)){
  ...
  avgGlobalRate <- mean(sec.frame$est)
  ...
  ggplot(sec.frame, aes(x=Lineage, y=est, group=rep)) + 
    ...
    annotate("text", x=13, y=avgGlobalRate*1.05, label = "Global rate", color = "#6F8FAF") +
    annotate("text", x=12.9, y=avgVOIRate*0.95, label = "VOI rate", color = "#D2B04C") +
