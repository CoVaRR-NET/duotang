...
reference <- list("BA.5.2.1")
startpar2 <- list(p=0.5, s=0.05)
startdate<-as.Date("2022-05-05")
fullpangotree=makepangotree(unique(meta$rawlineage[meta$sample.collection.date>as.Date("2022-08-31") ]))

allparams=list()
for( l in fullpangotree){
...
plotsubvariant <- function(region,min,node){
  gg=getsublineagesfromnode(node)
  for(p in allparams) {
    if(!any(is.na(p$fit))){
      if(p$region==region && (p$fit)$fit[["s1"]]>0 && sum(p$toplot$n2)>min){
          printest(p$mut[[1]])
