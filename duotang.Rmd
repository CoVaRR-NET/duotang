...
all.regions <- c("East provinces (NL+NS+NB)", "Alberta", "Manitoba", #"Manitoba", 
                 "Saskatchawan", "Quebec", "Ontario", "Canada")

#t0 <- Sys.time()
allparams <- mclapply(latelineage_pangotree, function(l) {
  lineagelist <- getStrictoSubLineages(l, meta)
  #for( region in rev(all.regions)){
  lapply(rev(all.regions), function(region) {
...
BQ.1.1 and some associated BQ variants are dominant at this time in Canada. Other lineages showing notable growth internationally are variants, such as XBB.1.5, with the spike mutation S:F486P. This mutation is of note as it potentially increases immune evasion while also improving binding with the human ACE2 receptor. S:F486P is a “two-step mutation”, requiring two changes from the Wuhan ancestor to transition from the amino acid phenylalanine (F) to proline (P) and is now observed in XBB.1.5 plus some BA.2/BA.2.75-based variants and other unrelated BA.5-based variants. Some intermediate variants with a serine (S) have also recently been observed, including in Canada, that may lead to more variants with S:F486P. Other variants are starting to increase notably internationally, plus in some regions of Canada, including XBB variants such as XBB.1.5, plus associated new BA.2/BA.2.75-based variants, plus some unrelated BA.5-based variants. Mutations such as S:F486P (associated with potentially improved binding with the human ACE2 receptor and immune evasion) are of particular interest.
Variants not yet classified and/or not yet in Canada that are associated with significant new outbreaks in other countries are also being monitored, including by mutation-based analysis.
**Variants of current interest**, due to their current/potential growth advantage, mutations of potential functional significance, or spread in other countries (See also the evolving plot below of “Fastest growing lineages in Canada”):

* BA.2 and BA.2.75 new subvariants not yet further classified
...
* BQ.1, BQ.1.1, BQ.1.3 and many associated descendants
* BW.1, BW.1.1 (descendants of BA.5.6)
* CK.1, CK.2.1.1 (descendants of BA.5.2)
* XBB newer variants, including XBB.1, XBB.1.5
...
### early BA.2
#### Sublineage BA.2
```{r subtype plotter BA.2 early, figures-side, message=FALSE, warning=FALSE, echo = FALSE, fig.width=5, fig.height=5, fig.show="hold", out.width="50%"}
set=getStrictoSubLineages("BA.2*",meta)
rarelineages_names=plot.subvariants(sublineage=set,maxdate=as.Date("2022-08-01"))
rarelineages_names=plot.subvariants(sublineage=set,maxdate=as.Date("2022-08-01"),scale=T)
printest(rarelineages_names)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
### late BA.2
#### Sublineage BA.2
```{r subtype plotter BA.2 late, figures-side, message=FALSE, warning=FALSE, echo = FALSE, fig.width=5, fig.height=5, fig.show="hold", out.width="50%"}
set=getStrictoSubLineages("BA.2*",meta)
rarelineages_names=plot.subvariants(sublineage=set,mindate=as.Date("2022-08-01"))
rarelineages_names=plot.subvariants(sublineage=set,mindate=as.Date("2022-08-01"),scale=T)
printest(rarelineages_names)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
plotsubvariant <- function(region,min,parentalnode){
  nplot=0
  for(p in allparams) {
    if(!any(is.na(p$fit)) && p$region==region && isSubLineage(parentalnode,p$mut[[1]])){
