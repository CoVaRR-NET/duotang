...
VOCVOI <- data.frame(
  name=c('Alpha', 'Beta', 'Gamma',
         ...
         'Lambda', 'Omicron BA.1', 'Omicron BA.1.1',
         'Omicron BA.2', 'Omicron BA.4', 'Omicron BA.5',
         'Mu', 'A.23.1', 'B.1.438.1'),
  pangodesignation=c('B.1.1.7|B.1.1.7.*', 'B.1.351|B.1.351.*', 'B.1.1.28|B.1.1.28.*',
            'B.1.617.2', 'B.1.617.2.25|B.1.617.2.25.*', 'B.1.617.2.27|B.1.617.2.27.*',
            'B.1.1.1|B.1.1.1.*', 'B.1.1.529.1|B.1.1.529.1.1[1-9]*|B.1.1.529.1.[2-9]*', 'B.1.1.529.1.1|B.1.1.529.1.1.*',
            'B.1.1.529.2|B.1.1.529.2.*', 'B.1.1.529.4|B.1.1.529.4.*', 'B.1.1.529.5|B.1.1.529.5.*',
            'B.1.621|B.1.621.*', 'A.23.1|A.23.1.*', 'B.1.438.1|B.1.438.1.*'),
  color=c('#B29C71', '#F08C3A', '#444444',
          '#A6CEE3', '#61A6A0', '#438FC0',
          '#CD950C', '#8B0000', '#CC6633',
...
* BA.5.2.1
* BA.5.6
* Possibly BF.1 and BF.5 (low numbers)
...
plot.selection.estimate(region="British Columbia", startdate=startdate, startpar=startpar3,
                        reference=c(sublineages_BA2), mutants=list(sublineages_BA4, sublineages_BA5),
                        names=list("BA.4*","BA.5*"), col=col)
```
...
print("Not enough data available")
plot.selection.estimate(region="Ontario", startdate=startdate, startpar=list(p=c(0.4, 0.5), s=c(0.05, 0.05)),
                        reference=c(sublineages_BA2), mutants=list(sublineages_BA4,sublineages_BA5),
                        names=list("BA.4*",""), col=col)
```
...
plot.selection.estimate(region="Quebec", startdate=startdate, startpar=list(p=c(0.4, 0.5), s=c(0.05, 0.05)),
                        reference=c(sublineages_BA2), mutants=list(sublineages_BA4, sublineages_BA5),
                        names=list("BA.4*","BA.5*"), col=col)
```
...
```{r general settings for selection estimator BA.2 , echo=FALSE, warning=FALSE, message=FALSE, fig.height=5, fig.show="hold", fig.width=5, warning=FALSE, out.width="33%", results='asis'}
startpar2 <- list(p=0.05, s=0.05)
...
```{r testing functions to get lineages}
...
Here we show the trends of the various BA.5# sublineages over time, relative to the frequency of BA.5 by itself (shown for sublineages with at least 50 (Canada) or 20 (provinces) cases). Proportions shown here are only among BA.5 (stricto) and the lineage illustrated. Note that these plots are not necessarily representative of trends in each province and that mixing of data from different provinces may lead to  shifts in frequency that are not due to selection.
...
temp <- sublineagedata  %>% filter(lineage!= "BA.5.1.7")
multi.plot.selection(temp,"Canada",namereference,50)
