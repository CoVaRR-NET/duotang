...
author: "Pillar 6"
date: '`r format(as.Date("2022-11-03"), "%B %d, %Y")`'
output:
  html_document:
    keep_md: true
    code_folding: hide
...
maxnumberofgraphperselectionplot=6
```
...
**Variants of current interest**, due to their potential growth advantage, or mutations of potential functional significance (with sublineages of many of these also of interest, such as all BQ*). See also the plot below of "Recently designated lineages showing most growth":
...
sublineages_BA51 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5\\.1$|B\\.1\\.1\\.529\\.5\\.1\\.", meta$rawlineage)])  # "BA.5.1" 
sublineages_BA52 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5\\.2$|B\\.1\\.1\\.529\\.5\\.2\\.", meta$rawlineage)])  # "BA.5.2" 
sublineages_BA5 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.", meta$rawlineage)])  # "BA.5" 
restofBA5=sublineages_BA5[!sublineages_BA5 %in% c(sublineages_BA51,sublineages_BA52)]
rest <- unique(meta$lineage)
rest=rest[!rest %in% c(sublineages_BA4,sublineages_BA2,sublineages_BA1,sublineages_BA5)]
...
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA51))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA51),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA52))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA52),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
rarelineages_names=plot.subvariants(sublineage=c(restofBA5))
rarelineages_names=plot.subvariants(sublineage=c(restofBA5),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
rarelineages_names=plot.subvariants(sublineage=c(rest),mindate=as.Date(VirusSeq_release)-days(200))
rarelineages_names=plot.subvariants(sublineage=c(rest),mindate=as.Date(VirusSeq_release)-days(200),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
sublineages_BA4 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.", meta$rawlineage)])  # "BA.4" 
sublineages_BA5 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.54\\.", meta$rawlineage)])  # "BA.5" 
...
fullpangotree=makepangotree(unique(meta$rawlineage[meta$sample.collection.date>startdate]))


is_not_knitr = function() {
        is.null(knitr::opts_knit$get("out.format"))
}
is_not_knitr()


allparamnonstricto=list()
for( l in fullpangotree[grepl("\\*",fullpangotree)]){
  lineagelist=getsublineagesfromnode(l)
  if(length(intersect(reference,lineagelist))==0){
  if(is_not_knitr()){
   print(paste(mut,"non stricto Canada")) 
  }
    if(! l %in% list("B.1.1.529.2.38.3*")){
      p=estimate.selection(region="Canada", startdate=startdate, reference=reference, mutants=lineagelist, startpar=startpar2)
      allparamnonstricto=append(allparamnonstricto,list(p))
    }
  }
}

allparamstricto=c()
for( region in rev(c("East provinces (NL+NS+NB)","Manitoba","Saskatchawan","Quebec","Ontario", "Canada"))){
  lineagelist=chooselineagestoplot(togrep=".",
                  region=region,
                  namereference=reference,
                  startdate=as.Date(startdate),
                  lastseenafter=date(VirusSeq_release)-days(100),
                  minnumberofsequence=10)
  if(region=="Quebec"){
    lineagelist=lineagelist[lineagelist!="BA.1.1"]
  }
  if(region=="Canada"){
    lineagelist=lineagelist[lineagelist!="BA.2.10.1"]
    lineagelist=lineagelist[lineagelist!="BF.3"]
  }
  for (mut in lineagelist){
    ...
    if(any(is.na((plot_param$fit)$sample))){
      ...
      allparamstricto=append(allparamstricto,list(plot_param))
    }
  }
}


  #sort by decreasing selection estimate
allparamstricto=allparamstricto[order(sapply(allparamstricto, function(p){(p$fit)$fit[["s1"]]}), decreasing = TRUE)]
allparamnonstricto=allparamnonstricto[order(sapply(allparamnonstricto, function(p){(p$fit)$fit[["s1"]]}), decreasing = TRUE)]
...
d = plot_growing_lineage(paramselected)
...
paramselected=allparamnonstricto[sapply(allparamnonstricto,function(x){x$region=="Canada" & (x$fit)$fit[["s1"]]>0 & sum(x$toplot$n2)>10})]
d = plot_growing_lineage(paramselected)
...
allparams=c(allparamnonstricto,allparamstricto)
paramselected=allparams[sapply(allparams,function(x){(x$fit)$fit[["s1"]]>0})]
d = plot_growing_lineage(paramselected,makeplot=FALSE)
DT::datatable(d)
...
plotba2 <- function(region,min){
  lineagelist=chooselineagestoplot(
    togrep="B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.|XBB",
    region=region,
    namereference=reference,
    startdate=as.Date(startdate),
    lastseenafter=date(VirusSeq_release)-days(100),
    minnumberofsequence=min)
  countgraph=0
  for(p in allparams) {
    if((p$region==region) & (p$mut[1] %in% lineagelist)){
      if((countgraph<maxnumberofgraphperselectionplot) | (p$fit)$fit[["s1"]]>0){
        print(p$mut)
...
plotba4 <- function(region,min){
  lineagelist=chooselineagestoplot(
    togrep="B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.",
    region=region,
    namereference=reference,
    startdate=as.Date(startdate),
    lastseenafter=date(VirusSeq_release)-days(100),
...
plotba5 <- function(region,min){
  lineagelist=chooselineagestoplot(
    togrep="B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.",
    region=region,
    namereference=reference,
    startdate=as.Date(startdate),
    lastseenafter=date(VirusSeq_release)-days(100),
