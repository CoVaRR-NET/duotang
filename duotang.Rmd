...
**[NOTE: Added a disclaimer above that the page below may take up to 30 seconds to load. Also, fixed the values for the selection coefficients to not be given on a percent basis.]**

XBB.1.5 is dominant in all regions of Canada. Other variants with the mutation S:F486P, in particular XBB.1.9.1 and other select subvariants of XBB.1.9, or XBB.1.16, continue to grow and include some consistent growth advantages over XBB.1.5. Some other select subvariants of XBB.1.5, BQ.1 and BA.2.75 are also increasing in some regions of Canada, but not as significantly or with lower numbers. Some mutations of recent note are more challenging to track, since they are in overlapping ORFs with different levels of impact on each ORF (i.e. mutation ORF9b:I5T which causes a synonymous change in an overlapping ORF that tends to be more tracked). Differences between some provinces continue but are less pronounced now due to the recent prevalence of XBB.1.5 - see the evolving plots below of “Fastest growing lineages in Canada”, and “case count trends by variant”, per province.

**Variants of current interest**, due to their current/potential growth advantage, mutations of potential functional significance, or spread in other countries (note that some of these are not yet detected in Canada. Italics denote variants/mutations of interest added since the last update of this summary, but they do not necessarily represent the primary variants of interest):
