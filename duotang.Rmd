...
```
The grey zone represent rare lineages (not in top 20) : `r rarelineages_names`
...
for (name2 in includelineages) {
  plot.selection(region="Canada",startdate=startdate, reference=name1, mutants=name2, startpar=startpar2)
}
```
...
sublineagedata <- meta %>% filter(
    lineage %in% unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.[0-9]", meta$rawlineage)]), 
    geo_loc_name..state.province.territory. %in% unique(meta$geo_loc_name..state.province.territory.),
    !is.na(sample.collection.date),
    sample.collection.date >= startdate
...
showlineages <- showlineages %>% group_by(lineage) %>% summarise(n = sum(n))
showlineages <- showlineages %>% filter(n>50)
#We'll use BA.2 as the reference and drop it:
showlineages <- showlineages %>% filter(lineage!="BA.2")
includelineages <- as.list(showlineages$lineage)
...
if (length(includelineages)<1){
  print("Not enough data available")
 } else {
    for (name2 in includelineages) {
      plot.selection(region="British Columbia", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
}
```
...
if (length(includelineages)<1){
  print("Not enough data available")
    ...
    for (name2 in includelineages) {
      plot.selection(region="Ontario", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
}
```
...
if (length(includelineages)<1){
  print("Not enough data available")
    ...
    for (name2 in includelineages) {
      plot.selection(region="Quebec", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
}
```
...
if (length(includelineages)<1){
  print("Not enough data available")
    ...
    for (name2 in includelineages) {
      plot.selection(region="Saskatchawan", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
}
```
...
if (length(includelineages)<1){
  print("Not enough data available")
    ...
    for (name2 in includelineages) {
      plot.selection(region="Manitoba", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
}
```
...
if (length(includelineages)<1){
  print("Not enough data available")
    ...
    for (name2 in includelineages) {
      plot.selection(region="East provinces (NL+NS+NB)", startdate=startdate, reference=name1, mutants=name2, startpar=startpar2) 
  }
 }
