...
These analyses represent only a snapshot of SARS-CoV-2 evolution in Canada. Only some infections are detected by PCR testing, only some of those are sent for whole-genome sequencing, and not all sequences are posted to public facing repositfa
tories.
...
#reference <- getsublineagesfromnode("B.1.1.529.5.2*")
...
for( l in rev(fullpangotree[grepl("\\*",fullpangotree)])){
  ...
  if(length(intersect(reference,lineagelist))==0){
   printest(paste(l,"non stricto Canada (",rawtoreallineage(l),")")) 
    if(! l %in% list("B.1.1.529.2.75.3.4.1*","B.1.1.529.2.10.1*","B.1.1.529.2.75.9*")){
...
for( region in rev(c("East provinces (NL+NS+NB)","Alberta","Manitoba","Manitoba","Saskatchawan","Quebec","Ontario", "Canada"))){
  lineagelist=chooselineagestoplot(togrep=".",
                  ...
                  startdate=date(VirusSeq_release)-days(80),
                  lastseenafter=date(VirusSeq_release)-days(40),
                  minnumberofsequence=10)
  for (mut in lineagelist){
    printest(paste(mut,region))
    ...
    if(any(is.na((plot_param$fit)$sample))){
      printest("plot_param$sample have NA")
      printest(plot_param$mut)
    }else{
...
plot_param=estimate.selection(region="Canada", startdate=startdate, reference=reference, mutants=list("BA.2.10.1"), startpar=startpar2)
...
getsublineagesfromnode("BR*")
meta %>% filter(lineage == "BR.1")
```
...
set=getsublineagesfromnode("BA.1*")
...
set=getsublineagesfromnode("BA.2*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
set=getsublineagesfromnode("BA.4*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
set=getsublineagesfromnode("BA.5.1*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
set=getsublineagesfromnode("BA.5.2*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
set=getsublineagesfromnode("BA.5*")
set=set[!set %in% append(getsublineagesfromnode("BA.5.1*"),getsublineagesfromnode("BA.5.2*"))]
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
set=getsublineagesfromnode("*")
set=set[!set %in% c(getsublineagesfromnode("BA.1*"),getsublineagesfromnode("BA.2*"),getsublineagesfromnode("BA.4*"),getsublineagesfromnode("BA.5*"))]
...
allparamnonstricto=allparamnonstricto[order(sapply(allparamnonstricto, function(p){(p$fit)$fit[["s1"]]}), decreasing = TRUE)]
paramselected=allparamnonstricto[sapply(allparamnonstricto,function(x){x$region=="Canada" & (x$fit)$fit[["s1"]]>-1 & sum(x$toplot$n2)>10})]
d = plot_growing_lineage(paramselected)
...
sublineages_BA2 <- getsublineagesfromnode("BA.2*")
sublineages_BA4 <- getsublineagesfromnode("BA.4*")
sublineages_BA5 <- getsublineagesfromnode("BA.5*")
...
plotsubvariant <- function(region,min,togrep){
...
plotba2 <- function(region,min){
  plotsubvariant(region,min,"B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.|XBB")
}


plotba4 <- function(region,min){
  plotsubvariant(region,min,"B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.")
}


plotba5 <- function(region,min){
  plotsubvariant(region,min,"B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.")
}
