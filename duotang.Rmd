...
for( region in c("East provinces (NL+NS+NB)","Manitoba","Saskatchawan","Quebec","Ontario", "Canada")){
  ...
  if(region=="Canada"){
    canadabug=c("BA.2.13","BA.2.18","BA.1","BA.2.3.20","BA.2.40.1","BA.2.7","BA.5.1.4")
    lineagelist=lineagelist[! lineagelist %in% canadabug]
  }
  if(region=="Ontario"){
    lineagelist=lineagelist[lineagelist!="BA.2.18"]
  }
  allparams=c(allparams,multi.plot.selection(lineagelist,reference,region,makeplot=FALSE))
}

allparams=allparams[order(sapply(allparams, function(p){(p$fit)$fit[["s1"]]}), decreasing = TRUE)]
...
paramselected=allparams[sapply(allparams,function(x){x$region=="Canada" & (x$fit)$fit[["s1"]]>0})]
