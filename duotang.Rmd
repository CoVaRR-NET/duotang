...
VOCVOI <- data.frame(
  name=c('Alpha', 'Beta', 'Gamma',
         ...
         'Lambda', 'Omicron BA.1', 'Omicron BA.2',
         'Mu', 'A.23.1', 'B.1.438.1',
         'Omicron BA.4', 'Omicron BA.5'),
  pangodesignation=c('B.1.1.7', 'B.1.351', 'B.1.1.28',
            'B.1.617|B.1.617.*', 'B.1.617.25', 'B.1.617.27',
            'B.1.1.1|B.1.1.1.*', 'B.1.1.529.1|B.1.1.529.1.*', 'B.1.1.529.2|B.1.1.529.2.*',
            'B.1.621', 'A.23.1', 'B.1.438.1',
            'B.1.1.529.4|B.1.1.529.4.*', 'B.1.1.529.5|B.1.1.529.5.*'),
  color=c('#B29C71', '#F08C3A', '#444444',
          '#A6CEE3', '#61A6A0', '#438FC0',
          '#CD950C', '#8B0000', '#FF0000',
          '#BB4513', '#9AD378', '#3EA534',
          '#900D09', '#BC544B')
) 
VOCVOI$pattern=gsub("\\.","\\\\.",VOCVOI$pangodesignation)
VOCVOI$pattern=gsub("^","^",VOCVOI$pattern)
VOCVOI$jstrick=apply( VOCVOI[ , c("name","color") ] , 1 , paste , collapse = "-" )
VOCVOI$jstrick=gsub("#","|",VOCVOI$jstrick)

variants <- sapply(VOCVOI$pattern, function(p) 
  grepl(p, meta$lineage, perl=T))
paste(VOCVOI$pattern,gsub(VOCVOI$color))

# verify that every row matches either 0 or 1 patterns
# table(apply(variants, 1, sum))

meta$jstrick <- 'other|grey'
meta$jstrick[apply(variants, 1, sum)==1] <- VOCVOI$jstrick[unlist(apply(variants, 1, which))]
meta$jstrick <- as.factor(meta$jstrick)


meta$pango.group <- 'other'
meta$pango.group[apply(variants, 1, sum)==1] <- VOCVOI$name[unlist(apply(variants, 1, which))]
meta$pango.group <- as.factor(meta$jstrick)
...
showlineages <- showlineages %>% filter(n>20)
#We'll use BA.2 as the reference and drop it:
showlineages <- showlineages %>% filter(lineage!="BA.2")
includelineages <- as.list(showlineages$lineage)
source("scripts/plot_selection_estimator_2type.R")
