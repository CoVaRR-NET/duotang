...
[Note some new analyses have been incorporated, including case count trends by variant.]

XBB.1.5 continues to grow steadily and significantly, and is now the most common lineage in some regions of Canada. Some other select variants with the S:F486P mutation are also showing growth in some regions, or need to be tracked due to their international growth, such as XBB.1.9.1 and XBF. BQ.1.1 and some subvariants are increasing again in some regions of Canada along with some other select BQ.1 and CH.1.1 subvariants. Differences between some provinces continue - see also the evolving plots below of “Fastest growing lineages in Canada”, per province.
...
* XBB.1, XBB.1.5, XBB.1.9.1, XBB.1.10, XBB.1.12, XBB.6.1 and subvariants with S:F486P (or other mutations of note like S:Y453F)
...
* …and other variants with mutations associated with immune evasion or higher binding affinity to the human ACE2 receptor (examples: S:K147N, S:R346T, S:R403K, S:Y453F, S:F486P; S:Q613H), particularly those variants with sets of immune evasive mutations of interest.
...
Growth advantage of 0-5% corresponds to doubling times of more than two weeks, with 5-10% reflecting s one to two week doubling time and over 10% representing significant growth of less than one week doubling time. Note that estimating selection of sub-variants with low sequence counts (less than 100 dots) is prone to error, such as mistaking one-time super spreader events or pulses of sequence data from one region as selection. These estimates should with lower sequence counts in one region should be considered as very preliminary.
...
```{r selection, child="selection.Rmd"}
```
...
```{r function for displaying lineage informaiton, warning=FALSE, message=FALSE}
source("scripts/scanlineages.R")
...
DT::datatable(lineageDescriptions, filter = 'top', rownames = F, options = list( dom='ip',pageLength = 5, autoWidth = TRUE))
```
