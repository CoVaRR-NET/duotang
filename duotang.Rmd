...
#                        reference=c(sublineages_BA2), mutants=list(sublineages_BA4, sublineages_BA5),
...
sublineagedata <- meta %>% filter(
    lineage %in% unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.", meta$rawlineage)]), 
...
source("scripts/plot_selection_estimator_2type.R")
showlineages <- sublineagedata %>% filter(lineage!="BA.2.75")
multi.plot.selection(showlineages,"Canada",namereference,50)
...
showlineages <- showlineages %>% filter(lineage!="BG.2")
showlineages <- showlineages %>% filter(lineage!="BA.2.9")
showlineages <- showlineages %>% filter(lineage!="BA.2.76")
...
Here we show the trends of the various BA.4# sublineages over time, relative to the frequency of BA.4 by itself (shown for sublineages with at least 50 (Canada) or 20 (provinces) cases). Proportions shown here are only among BA.2 (stricto) and the lineage illustrated. Note that these plots are not necessarily representative of trends in each province and that mixing of data from different provinces may lead to  shifts in frequency that are not due to selection.
...
sublineagedata <- meta %>% filter(
  lineage %in% unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.", meta$rawlineage)]), 
  geo_loc_name..state.province.territory. %in% unique(meta$geo_loc_name..state.province.territory.),
  !is.na(sample.collection.date),
  sample.collection.date >= startdate
) %>% group_by(sample.collection.date,geo_loc_name..state.province.territory.) %>% dplyr::count(lineage)
...
Here we show the trends of the various BA.5# sublineages over time, relative to the frequency of BA.5.1 by itself (shown for sublineages with at least 50 (Canada) or 20 (provinces) cases). Proportions shown here are only among BA.5.1 (stricto) and the lineage illustrated. Note that these plots are not necessarily representative of trends in each province and that mixing of data from different provinces may lead to  shifts in frequency that are not due to selection.
...
sublineagedata <- meta %>% filter(
  lineage %in% unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.", meta$rawlineage)]), 
  geo_loc_name..state.province.territory. %in% unique(meta$geo_loc_name..state.province.territory.),
  !is.na(sample.collection.date),
  sample.collection.date >= startdate
) %>% group_by(sample.collection.date,geo_loc_name..state.province.territory.) %>% dplyr::count(lineage)
source("scripts/plot_selection_estimator_2type.R")
namereference <- "BA.5.1"
...
sec.frame=sec.frame[sec.frame$Lineage != "others",]
