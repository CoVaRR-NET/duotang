...
lineageDescriptions <- read.table(paste0(params$datadir,"lineageNotes.tsv"), sep = "\t", header=T, as.is=TRUE, quote = "", stringsAsFactors = F, comment.char = "", fill = T) %>% 
  ...
  dplyr::select(Lineage, Parent, Description) %>% mutate(`Present in Canada Last 120 days` = ifelse(Lineage %in% uniqueNamedLineageLast120Days, length(meta$lineage[meta$lineage == Lineage & meta$sample_collection_date > startdate]), 0))
colnames(lineageDescriptions) <- c("Lineage", "Ancestor", "Description", "# In Canada Last 3Mo")
...
Therefore, from this time onward, case counts are likely underestimated and the sequenced virus diversity is not necessarily representative of the virus circulating in the overall population [@sally, comments and potential citaitons?]. 
...
individualSelectionPlotReference <- getStrictoSubLineages("HV.1", meta)
...
The image below is a screenshot from VIRUS-MVP. please click to examine mutations and their curated functional information. Shows a snapshot of the VIRUS-MVP tool of lineages actively circulating in Canada. To scan the entire genome and read more details, click on the image below or see https://virusmvp.org/covid-mvp.
...
Because genomics data often lag case count data, the last date of reliable case count data may be closer to the present than the last date of genomics data. In these cases, the evolutionary model for $p_i(t)$ is projected forward in time and shown as a ligher shaded area under the curve within the plot).
...
Additional COVID-19 information for British Columbia can be found here: http://www.bccdc.ca/health-info/diseases-conditions/covid-19

### AB
#### Alberta
Additional COVID-19 information for Alberta can be found here: https://www.alberta.ca/coronavirus-info-for-albertans


### SK
#### Saskatchewan
Additional COVID-19 information for Saskatchewan can be found at: https://www.saskatchewan.ca/government/health-care-administration-and-provider-resources/treatment-procedures-and-guidelines/emerging-public-health-issues/2019-novel-coronavirus/cases-and-risk-of-covid-19-in-saskatchewan


### MB
#### Manitoba
Additional COVID-19 information for Manitoba can be found at: https://www.gov.mb.ca/health/publichealth/surveillance/influenza/index.html


### ON
#### Ontario
Additional COVID-19 information for Ontario can be found at: https://www.publichealthontario.ca/en/Data-and-Analysis/Infectious-Disease/Respiratory-Virus-Tool

### QC
#### Quebec
Quebec provides a list of updates on changes to screening and sequencing strategies, found here (in French): https://www.inspq.qc.ca/covid-19/donnees/variants#methodologie.
Additions up-to-date COVID-19 data for Quebec can be found at: https://www.inspq.qc.ca/covid-19/donnees/variants

### NS
#### Nova Scotia
Additional COVID-19 information for Nova Scotia can be found at: https://novascotia.ca/coronavirus/alerts-notices/#epidemiologic-summaries


### NB
#### New Brunswick
Additional COVID-19 information for New Brunswick can be found at: https://www2.gnb.ca/content/gnb/en/corporate/promo/respiratory-watch.html


### NL
#### Newfoundland and Labrador
Additional COVID-19 information for Newfoundland and Labrador can be found at: https://experience.arcgis.com/experience/b47262b1265f48ba95d7477c56835593
...
* UShER: Ultrafast Sample placement on Existing tRee - for placing a small-ish dataset into the global GISAID phylogenetic tree [web-version](https://genome.ucsc.edu/cgi-bin/hgPhyloPlace), [local-version]( https://shusher.gi.ucsc.edu) [unavailable]
...
* List of [homoplaises in SARS-CoV-2](https://github.com/corneliusroemer/ncov-simplest/blob/main/data/exclude_sites_light.txt) [Last updated 3yr ago]
* Erin Gill's [COVID-19 dashboard](https://github.com/eringill/COVID_dashboard_reboot) [not updated?]
...
* [Nybbler tool](https://github.com/nodrogluap/nybbler) for subsampling SARS-CoV-2 genome ensembles [last updated 3yr ago]
