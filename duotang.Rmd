...
### others BA.5
#### Sublineage BA.5 without BA.1 and BA.2
...
allparamstricto=c()
for( region in c("East provinces (NL+NS+NB)","Manitoba","Saskatchawan","Quebec","Ontario", "Canada")){
  ...
  #if(region=="Ontario"){
  #  lineagelist=lineagelist[lineagelist!="BA.2.18"]
  #  lineagelist=lineagelist[lineagelist!="BA.2.75"]
  #}
...
```{r overview of the selection estimator plot stricto, fig.retina = 2, warning=FALSE, fig.height=7}

source("scripts/plot_growing_lineages.R")
paramselected=allparamstricto[sapply(allparamstricto,function(x){x$region=="Canada" & (x$fit)$fit[["s1"]]>0 & sum(x$toplot$n2)>49})]
...
source("scripts/plot_growing_lineages.R")
paramselected=allparamnonstricto[sapply(allparamnonstricto,function(x){x$region=="Canada" & (x$fit)$fit[["s1"]]>0 & sum(x$toplot$n2)>49})]
for(p in paramselected){
  print(paste(p$mut[1],sum(p$toplot$n2)))
}
d = plot_growing_lineage(paramselected)
...
plotba4 <- function(region,min){
  ...
  countgraph=0
  for(p in allparams) {
    if(p$region==region & p$mut %in% lineagelist){
      if((countgraph<maxnumberofgraphperselectionplot) | (p$fit)$fit[["s1"]]>0){
        plot.selection(plotparam=p)
        countgraph=countgraph+1
      }
