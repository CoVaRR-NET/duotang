...
author: "Pillar 6"
date: '`r format(as.Date("2022-10-20"), "%B %d, %Y")`'
output:
  html_document:
    keep_md: true
    code_folding: hide
...
VOCVOI = data.frame(name=character(),
                     pangodesignation=character(),
                     color=character())
addVOCVOI <-  function(n,p,c) {return(rbind(VOCVOI,data.frame(name=n,pangodesignation=p,color=c)))}

VOCVOI=addVOCVOI("Alpha","B.1.1.7|B.1.1.7.*",'#B29C71')
VOCVOI=addVOCVOI("Beta","B.1.351|B.1.351.*",'#F08C3A')
VOCVOI=addVOCVOI("Gamma","B.1.1.28|B.1.1.28.*",'#0000FF')
VOCVOI=addVOCVOI("Delta","B.1.617.2|B.1.617.2.*",'#A6CEE3')
VOCVOI=addVOCVOI("Lambda","B.1.1.1|B.1.1.1.*",'#FFFF00')

VOCVOI=addVOCVOI("Omicron BA.1","B.1.1.529.1|B.1.1.529.1.*",'#8B0000')
VOCVOI=addVOCVOI("Omicron BA.2","B.1.1.529.2|B.1.1.529.2.*",'#FF0000')
VOCVOI=addVOCVOI("Omicron BA.4","B.1.1.529.4|B.1.1.529.4.*",'#BC544B')
VOCVOI=addVOCVOI("Omicron BA.5","B.1.1.529.5|B.1.1.529.5.*",'#CC66FF')

VOCVOI=addVOCVOI("A.23.1","A.23.1|A.23.1.*",'#9AD378')
VOCVOI=addVOCVOI("B.1.438.1","B.1.438.1|B.1.438.1.*",'#3EA534')
VOCVOI=addVOCVOI("Recombinants","X*",'black')
...
#Select on ly the column we want to use later
meta <- meta[,c("fasta.header.name","sample.collection.date","week",  "province", "rawlineage", "lineage")]
...
dat[['Canada']] <- as.data.frame.matrix(table(meta1$week, meta1$pango.group))
...
sublineagedata <- meta %>% filter(
    ...
    lineage!="BA.2.18",
    lineage!="BA.2.3.20",
    sample.collection.date >= startdate) %>% group_by(sample.collection.date) %>% dplyr::count(lineage)
...
lineagelist=c(namereference,unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.|XBB", meta$rawlineage)]))
sublineagedata <- meta %>% filter(
    lineage %in% lineagelist, 
    province %in% unique(meta$province),
    !is.na(sample.collection.date),
    sample.collection.date >= startdate
    ) %>% group_by(sample.collection.date,province) %>% dplyr::count(lineage)
...
showlineages <- sublineagedata %>% filter(province=="British Columbia")
...
showlineages <- sublineagedata %>% filter(province=="Ontario")
...
showlineages <- sublineagedata %>% filter(province=="Quebec")
...
showlineages <- sublineagedata %>% filter(province=="Saskatchawan")
...
showlineages <- sublineagedata %>% filter(province=="Manitoba")
...
print("Not enough data available")
#showlineages <- sublineagedata %>% filter(province=="New Brunswick"|province=="Nova Scotia" | province=="Newfoundland and Labrador")
...
#print("Not enough data available")
showlineages <- sublineagedata %>% filter(province=="New Brunswick"|province=="Nova Scotia" | province=="Newfoundland and Labrador")
showlineages <- showlineages %>% filter(lineage !="BA.2.12.1")
...
sublineagedata <- meta %>% filter(
  ...
  !is.na(sample.collection.date),
  sample.collection.date >= startdate
) %>% group_by(lineage,province) %>% summarise(count = n(), 
            lastsampledate = max(sample.collection.date))
...
source("scripts/plot_selection_estimator_2type.R")
multi.plot.selection(sublineagedata,"Canada",namereference,500)


sublineagedata <- sublineagedata[order(sublineagedata$sample.collection.date, decreasing = TRUE), ]

data_new1 <- Reduce(rbind,                                 # Top N highest values by group
                    by(sublineagedata,
                       sublineagedata["lineage"],
                       head,
                       n = 1))
...
showlineages <- sublineagedata %>% filter(province=="New Brunswick"|province=="Nova Scotia" | province=="Newfoundland and Labrador")
showlineages <- showlineages %>% filter(lineage!="BA.5.2.21")
showlineages <- showlineages %>% filter(lineage!="BA.5.2.20")
showlineages <- showlineages %>% filter(lineage!="BA.5.2.2")
showlineages <- showlineages %>% filter(lineage!="BA.5.2.9")
showlineages <- showlineages %>% filter(lineage!="BE.1.1")
#showlineages <- showlineages %>% filter(lineage!="BF.5")
showlineages <- showlineages %>% filter(lineage!="BF.7")
#showlineages <- showlineages %>% filter(lineage!="BF.28")
multi.plot.selection(showlineages,"East provinces (NL+NS+NB)",namereference,20)
```
