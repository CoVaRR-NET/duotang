...
columnlist=c("fasta.header.name", "province", "host.gender", "host.age.bin", "sample.collection.date", 
                      "sample.collected.by", "purpose.of.sampling", "purpose.of.sequencing","lineage", "rawlineage")
meta <- meta[,columnlist]


for(col in columnlist){
  meta[col][meta[col]=="Undeclared"] <- "Unknown"
  meta[col][meta[col]=="Not Provided"] <- "Unknown"
  meta[col][meta[col]=="Restricted Access"] <- "Unknown"
  meta[col][meta[col]=="Missing"] <- "Unknown"
  meta[col][meta[col]=="Not Applicable"] <- "Unknown"
}
meta["purpose.of.sequencing"][meta["purpose.of.sequencing"]=="Sample has epidemiological link to Omicron Variant"] <- "Research"
...
fullpangotree=makepangotree(unique(meta$rawlineage))
latelineage_pangotree=makepangotree(unique(meta$rawlineage[meta$sample.collection.date>as.Date("2022-10-01") ]))
...
for( l in latelineage_pangotree){
  lineagelist=getStrictoSubLineages(l)
...
BQ.1, BQ.1.1 and associated sublineages continue to show the strongest growth, with other newer sublineages as noted below also of potential interest.
...
set=getStrictoSubLineages("BA.1*")
...
set=getStrictoSubLineages("BA.2*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
printest(rarelineages_names)
```
...
set=getStrictoSubLineages("BA.5.1*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
printest(rarelineages_names)
```
...
set=getStrictoSubLineages("BA.5.2*")
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
printest(rarelineages_names)
```
...
set=getStrictoSubLineages("BA.5*")
set=set[!set %in% append(getStrictoSubLineages("BA.5.1*"),getStrictoSubLineages("BA.5.2*"))]
rarelineages_names=plot.subvariants(sublineage=set)
rarelineages_names=plot.subvariants(sublineage=set,scale=T)
printest(rarelineages_names)
```
...
set=set[!set %in% c(getStrictoSubLineages("BA.1*"),getStrictoSubLineages("BA.2*"),getStrictoSubLineages("BA.4*"),getStrictoSubLineages("BA.5*"))]
...
plot_growing_lineage(paramselected)
...
sublineages_BA2 <- getStrictoSubLineages("BA.2*")
sublineages_BA4 <- getStrictoSubLineages("BA.4*")
sublineages_BA5 <- getStrictoSubLineages("BA.5*")
...
plotsubvariant <- function(region,min,parentalnode){
  ...
  subnodes=getStrictoSubLineages(parentalnode)
  for(p in allparams) {
...
fieldnames<- c("fasta.header.name", "province", "host.gender", "host.age.bin", "sample.collection.date", 
...
metasub1 <- meta[meta$fasta.header.name %in% ttree$tip.label, fieldnames]
# sort rows to match tip labels in tree
metasub1 <- metasub1[match(ttree$tip.label, metasub1$fasta.header.name), ]
metasub_omi <- metasub1[grepl("Omicron",metasub1$pango.group ), ]
