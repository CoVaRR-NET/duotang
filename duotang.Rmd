...
```{r function, echo=FALSE, wanring=FALSE, message=FALSE}
...
```{r create sublineage selection plots,echo=FALSE, message=FALSE,warning=FALSE}
pl_list<-list()
for(i in includelineages){
  name1 <- unique(meta$lineage[grepl("BA\\.2$|BA\\.2\\.[0-9]|BA\\.2\\.[0-9][0-9]", meta$lineage)])
  name2 <- i
  name1 <- name1[!(name1 %in% c(name2))]
  myplots<-lapply(i,plot.selection.estimate_test(region="Canada", startdate=startdate, startpar=startpar2,
                       reference=c(name1), mutants=list(name2), col=col))
  #pl_list[[i]] <- list(sec_plot,i)
}
```
