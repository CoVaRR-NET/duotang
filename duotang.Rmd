...
list.of.packages <- c("tidyr", "knitr", "lubridate", "parallel", "ggplot2", "r2d3", "jsonlite", "tidyverse", "reshape2", "DT", "plotly", "splines", 'HelpersMG', "bbmle")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)>0) {
...
It is for discussion with pillar 6/CAMEO's team and for sharing with collaborators, e.g. public health labs.
...
individualSelectionPlotReference <- getStrictoSubLineages("XBB.1.5", meta)
...
**[Note: The Fastest Growing Lineages plot for Canada now enlarges a point if growing rapidly in multiple provinces. You can also now copy isolate IDs from tree nodes using (ctrl|cmd)+v.]**

The proportion of cases that contain the mutation S:F486P continues to grow in Canada and is now estimated at over 90%. Of these variants, XBB.1.5 still predominates at over 60% of samples, but other lineages that contain this 486P mutation are now growing that contain additional mutations noted for increased immune evasion (like S:F456L) or binding to the ACE2 receptor or infectivity (like S:P521S and S:T478R). These include variants and sub variants of XBB.1.16, XBB.1.5.10, XBB.1.5.15, XBB.1.9.1 and XBB.2.3. More differences in regions are noted, with for example XBB.1.16 still most notable in Ontario, and XBB.1.5.15 is most prevalent in Quebec. Subvariants with more combinations of these mutations are being tracked.

**Variants of current interest**, due to their current/potential growth advantage, mutations of potential functional significance, or spread in other countries (note that some may not yet be detected in Canada). **_Bold_** denotes those added since the last update of this summary, but they do not necessarily represent the primary variants of interest - the latter are usually highlighted in the text above):

* DV.1 (BA.2.75 descendant)
* EG.1 (an XBB.1.9.1 subvariant)
* EK variants (subvariants of XBB.1.5.13)
* **_FD.1.1 (subvariant of XBB.1.5.15)_**
* **_FE.1_**
* Multiple XBB.1.5 subvariants including **_XBB.1.5.10_** and XBB.1.5.15 **_and others that have S:F456L_**
...
* XBB.2.3 (has mutation S:P521S that could increase human ACE2 receptor binding/infectivity, **_with an interest in those also with mutation 478R)_**
...
Mutations of interest include:
  ...
  + S:E180V **_(though this may have a detrimental effect to XBB.1.16)_**
  + S:Y453F **_(could increase binding to the human ACE2 receptor)_**
  + **_S:F456L (evidence of increased immune evasion versus recent variants and a mutation growing in prevalence in multiple regions)_**
  + S:T478R (aka S:K478R - the S:T478K mutation occurred first). **_Evidence of increased infectivity when introduced into XBB.1.5. Associated with XBB.1.16 (which has additional mutations like S:E180V that may counteract this mutations advantage) but now seen in additional variants like XBB.2.3._**
  + S:P521S **_(evidence it could increase human ACE2 receptor binding/infectivity - associated with XBB.2.3 variants)_**
  + S:Q613H **_(growing and seen in XBB.1.5, CH.1.1 and XBB.1.9.1)_**
...
Here we take a look, sub-dividing the major sub-lineages currently circulating in Canada. (Click and drag to zoom, double click to reset.)
...
* [Outbreak Info](https://outbreak.info/situation-reports): SARS2 data explorer: lineage comparison, mutation tracker, etc
