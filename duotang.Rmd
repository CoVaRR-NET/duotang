...
VirusSeq_date="2022-10-03"
pangoversion="4.1.2 (Viral AI)"
...
VOCVOI <- data.frame(
  name=c('Alpha', 'Beta', 'Gamma',
         'Delta',
         'Lambda', 'Omicron BA.1',
         'Omicron BA.2', 'Omicron BA.4', 'Omicron BA.5',
         'A.23.1', 'B.1.438.1','Recombinants'),
         #'Mu', 'A.23.1', 'B.1.438.1'),
  pangodesignation=c('B.1.1.7|B.1.1.7.*', 'B.1.351|B.1.351.*', 'B.1.1.28|B.1.1.28.*',
            'B.1.617.2|B.1.617.2.*',
            'B.1.1.1|B.1.1.1.*', 'B.1.1.529.1|B.1.1.529.1.*',
            'B.1.1.529.2|B.1.1.529.2.*', 'B.1.1.529.4|B.1.1.529.4.*', 'B.1.1.529.5|B.1.1.529.5.*',
            'A.23.1|A.23.1.*', 'B.1.438.1|B.1.438.1.*', 'X*'),
            #'B.1.621|B.1.621.*', 'A.23.1|A.23.1.*', 'B.1.438.1|B.1.438.1.*'),
  color=c('#B29C71', '#F08C3A', '#0000FF',
          '#A6CEE3',
          '#FFFF00', '#8B0000',
          '#FF0000', '#BC544B','#CC66FF',
          '#9AD378', '#3EA534','black')
          #'#FFFF00', '#9AD378', '#3EA534')
)

VOCVOI$pattern <- gsub("\\.","\\\\.",VOCVOI$pangodesignation)
VOCVOI$pattern <- gsub("^","^",VOCVOI$pattern)
VOCVOI$pattern <- gsub("\\|","|^",VOCVOI$pattern)
VOCVOI$pattern <- gsub("([^\\*])\\|","\\1$|",VOCVOI$pattern)
VOCVOI$pattern <- gsub("([^\\*])$","\\1$",VOCVOI$pattern)
VOCVOI$pattern <- gsub("([\\*])$",".*",VOCVOI$pattern)
VOCVOI$pattern <- gsub("([\\*])\\|",".*|",VOCVOI$pattern)

variants <- sapply(VOCVOI$pattern, function(p) 
  grepl(p, meta$rawlineage, perl=T))
...
#To detect unassigned lineages due to wrong pattern matching
#print(unique(meta[meta$pango.group=="other",]$lineage))
#pb="B.1.617.2.25.1.2"
#print(variants <- sapply(VOCVOI$pattern, function(p) 
#  grepl(p, pb, perl=T)))
#print(grepl(VOCVOI$pattern[4], pb, perl=T))
...
#meta1$pango.group <- meta1$jstrick 
...
* CA.1
* XBB
* and other variants with immune evasion mutations, such as S:R346T (edited)
...
require(lubridate)
dateseq <- seq(ymd('2019-12-01'), ymd('2022-07-01'), by='3 month')
...
### omicron time tree
metasub_omi <- metasub1[grepl("B\\.1\\.1\\.529\\.", metasub1$rawlineage),]
...
sublineages_BA1 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.1$|B\\.1\\.1\\.529\\.1\\.", meta$rawlineage)]) # "BA.1"
sublineages_BA2 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.", meta$rawlineage)])  # "BA.2" 
#nameBA3 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.3$|B\\.1\\.1\\.529\\.3\\.", meta$rawlineage)])  # "BA.3" 
sublineages_BA4 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.", meta$rawlineage)])  # "BA.4" 
sublineages_BA5 <- unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.", meta$rawlineage)])  # "BA.5" 
...
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA1))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA1),scale=T)
```
...
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA2))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA2),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA4))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA4),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
### BA.5
#### Sublineage BA.5
```{r subtype plotter BA.5, figures-side, message=FALSE, warning=FALSE, echo = FALSE, fig.width=5, fig.height=5, fig.show="hold", out.width="50%"}

rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA5))
rarelineages_names=plot.subvariants(sublineage=c(sublineages_BA5),scale=T)
```
The grey zone represent rare lineages (not in top 15) : `r rarelineages_names`
...
```{r general settings for selection estimator, warning=FALSE, echo = FALSE, fig.width=5,fig.height=5, fig.show="hold", out.width="50%"}

source("scripts/plot_selection_estimator_2type.R")
namereference <- "BA.5.2"
startpar2 <- list(p=0.5, s=0.05)
startdate<-as.Date("2022-05-01") 

###for a cleaner scan of the sublineages (not in use now)
getsublineagelist <- function(mainlineage) {
  togrep=paste(mainlineage,"$|",mainlineage,".",sep="")
  list_of_lineage=unique((meta %>% filter(lineage %in% unique(meta$lineage[grepl(gsub("\\.", "\\\\.",togrep), meta$rawlineage)])))$lineage)
  return(list_of_lineage)
}

getsublineagesiblings <- function(mainlineage) {
  decompose=(strsplit(mainlineage, "\\."))[[1]]
  parent=sapply(list(head(decompose,-1)), paste, collapse = ".")
  grepmainlineage=paste(mainlineage,"$|",mainlineage,".",sep="")
  greparent=paste(parent,"|",parent,".",sep="")
  togrep=paste("^(?!",grepmainlineage,")(",greparent,")",sep="")
  list_of_lineage=unique((meta %>% filter(lineage %in% unique(meta$lineage[grepl(gsub("\\.", "\\\\.",togrep), meta$rawlineage, perl=T)])))$lineage)
  return(list_of_lineage)
}
...
#getsublineagelist(l)
#getsublineagesiblings(l)
#print(getsublineagelist("B.1.1.529.2.3"))
#plot.selection(region=region, startdate=startdate, reference=namereference, mutants="BA.2.3", #startpar=startpar2) 
#plot.selection(region=region, startdate=startdate, reference=namereference, mutants="BA.2.3", startpar=startpar2) 
#plot.selection(region=region, startdate=startdate, reference=getsublineagesiblings("B.1.1.529.2.3"), mutants="BA.2.3", startpar=startpar2) 
#plot.selection(region=region, startdate=startdate, reference=namereference, mutants=getsublineagelist("B.1.1.529.2.3"), startpar=startpar2) 

```

## Lineages showing the most growth in Canada {.tabset}


Here we show all the selection estimates and their 95% confidence intervals of the various pangolin lineages with more than 10 sequences after `r startdate`. We are show here with a positive upper bound of the confidence interval. Each estimate is done relative to `r namereference` stricto and the lineage. The graphs illustrating those estimates can be found below.

### Plot
#### Plot

Blue area show 0-5% ->modest growth (doubling time > two weeks)
Purple area show 5%-10% -> rapid growth likely to displace currently common strains (one to two week doubling times)
Red area 10%+ -> growth advantage characteristic of a variant of concern (over one week doubling time).

```{r overview of the selection estimator plot}
...
sublineagedata <- meta %>% filter(
    ...
    lineage != "BA.4.1.1",
    lineage != "BF.2"
    ) %>% group_by(sample.collection.date,geo_loc_name..state.province.territory.) %>% dplyr::count(lineage)
r=multi.plot.selection(sublineagedata,"Canada",namereference,10,makeplot=FALSE)
...
for(i in 1:length(r)){
  ...
  if(high_CI>0){
    ...
    d[nrow(d)+1, ] <-c(lineage = r[[i]]$mut,
                       sel_coeff=sel_coeff*100,
                       low_CI = low_CI*100,
                       high_CI =high_CI*100,
                       size = s,
                       sequence_count=paste(toString(10^round(log10(s)-0.5)),toString(10^round(log10(s)+0.5)),sep="-"))
  }
}
d$sel_coeff=as.numeric(d$sel_coeff)
d$low_CI=as.numeric(d$low_CI)
d$high_CI=as.numeric(d$high_CI)
d$sequence_count=as.factor(d$sequence_count)

d %>%  arrange(sel_coeff) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
mutate(lineage=factor(lineage, levels=lineage)) %>% 
ggplot(aes(x=lineage, y=sel_coeff, colour=sequence_count))+
geom_point(size=4)+
geom_pointrange( aes(ymin=low_CI, ymax=high_CI))+
geom_hline(yintercept=10, linetype="dashed", color = "grey")+ #dash line at 20% per day to mark 1st Omicron wave growth+
annotate("rect", xmin = 0, xmax = nrow(d)+1, ymin = 0, ymax = 5,
           alpha = .08,fill = "blue")+
annotate("rect", xmin = 0, xmax = nrow(d)+1, ymin = 5, ymax = 10,
           alpha = .08,fill = "purple")+
annotate("rect", xmin = 0, xmax = nrow(d)+1, ymin = 10, ymax = max(d$high_CI,11),
           alpha = .08,fill = "red")+
coord_flip()+ scale_color_brewer(palette = "Dark2")+
ggtitle("Recently designated lineages showing most growth")+ labs(x="", y= paste("growth advantage (% per day) relative to", namereference))+theme_bw()
...
Here we show the trends of the various BA.2.* sublineages over time, relative to the frequency of `r namereference` by itself (shown for sublineages with at least 50 (Canada) or 20 (provinces) cases). Proportions shown here are only among `r namereference` (stricto) and the lineage illustrated. Note that these plots are not necessarily representative of trends in each province and that mixing of data from different provinces may lead to  shifts in frequency that are not due to selection.

```{r general settings for selection estimator BA.2}
startpar2 <- list(p=0.5, s=0.05)
startdate<-as.Date("2022-05-05") 
lineagelist=c(namereference,unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.2$|B\\.1\\.1\\.529\\.2\\.", meta$rawlineage)]))
sublineagedata <- meta %>% filter(
    lineage %in% lineagelist, 
...
source("scripts/plot_selection_estimator_2type.R")
multi.plot.selection(sublineagedata,"Canada",namereference,50)
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="British Columbia")
multi.plot.selection(showlineages,"British Columbia",namereference,20)
```
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="Ontario")
multi.plot.selection(showlineages,"Ontario",namereference,20)
```
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="Quebec")
multi.plot.selection(showlineages,"Quebec",namereference,20)
```
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="Saskatchawan")
multi.plot.selection(showlineages,"Saskatchawan",namereference,20)
```
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="Manitoba")
multi.plot.selection(showlineages,"Manitoba",namereference,20)
```
...
print("Not enough data available")
#showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="New Brunswick"|geo_loc_name..state.province.territory.=="Nova Scotia" | geo_loc_name..state.province.territory.=="Newfoundland and Labrador")
#multi.plot.selection(showlineages,"East provinces (NL+NS+NB)",namereference,20)
```


## BA.4 sublineage selection {.tabset}

Here we show the trends of the various BA.4# sublineages over time, relative to the frequency of `r namereference` by itself (shown for sublineages with at least 50 (Canada) or 20 (provinces) cases). Proportions shown here are only among `r namereference` (stricto) and the lineage illustrated. Note that these plots are not necessarily representative of trends in each province and that mixing of data from different provinces may lead to  shifts in frequency that are not due to selection.

```{r general settings for selection estimator BA.4 , echo=FALSE, warning=FALSE, message=FALSE, fig.height=5, fig.show="hold", fig.width=5, warning=FALSE, out.width="33%", results='asis'}
startpar2 <- list(p=0.5, s=0.05)
lineagelist=c(namereference,unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.4$|B\\.1\\.1\\.529\\.4\\.", meta$rawlineage)]))
sublineagedata <- meta %>% filter(
  lineage %in% lineagelist, 
  geo_loc_name..state.province.territory. %in% unique(meta$geo_loc_name..state.province.territory.),
  !is.na(sample.collection.date),
...
#print("Not enough data available")
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="New Brunswick"|geo_loc_name..state.province.territory.=="Nova Scotia" | geo_loc_name..state.province.territory.=="Newfoundland and Labrador")
showlineages <- showlineages %>% filter(lineage !="BA.4.2")
multi.plot.selection(showlineages,"East provinces (NL+NS+NB)",namereference,20)
```
...
```{r general settings for selection estimator BA.5 , echo=FALSE, warning=FALSE, message=FALSE, fig.height=5, fig.show="hold", fig.width=5, warning=FALSE, out.width="33%", results='asis'}
startpar2 <- list(p=0.5, s=0.05)
startdate<-as.Date("2022-05-05")
lineagelist=c(namereference,unique(meta$lineage[grepl("B\\.1\\.1\\.529\\.5$|B\\.1\\.1\\.529\\.5\\.", meta$rawlineage)]))
sublineagedata <- meta %>% filter(
  lineage %in% lineagelist, 
  geo_loc_name..state.province.territory. %in% unique(meta$geo_loc_name..state.province.territory.),
  !is.na(sample.collection.date),
...
showlineages <- sublineagedata %>% filter(geo_loc_name..state.province.territory.=="New Brunswick"|geo_loc_name..state.province.territory.=="Nova Scotia" | geo_loc_name..state.province.territory.=="Newfoundland and Labrador")
source("scripts/plot_selection_estimator_2type.R")
#showlineages <- showlineages %>% filter(lineage!="BA.5.1.7")
showlineages <- showlineages %>% filter(lineage!="BF.7")
multi.plot.selection(showlineages,"East provinces (NL+NS+NB)",namereference,20)
```
