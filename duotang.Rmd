...
author: "Pillar 6"
date: '`r format(as.Date("2022-11-07"), "%B %d, %Y")`'
output:
  html_document:
    keep_md: true
    code_folding: hide
...
for( l in fullpangotree[grepl("\\*",fullpangotree)]){
  ...
  if(length(intersect(reference,lineagelist))==0){
   printest(paste(l,"non stricto Canada (",rawtoreallineage(l),")")) 
    if(! l %in% list("B.1.1.529.2.75.3.4.1*","B.1.1.529.5.2.1.3*")){
...
chooselineagestoplot(togrep=".",
                  region="Canada",
                  namereference=reference,
                  startdate=date(VirusSeq_release)-days(60),
                  lastseenafter=date(VirusSeq_release)-days(30),
