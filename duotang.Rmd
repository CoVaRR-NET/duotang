...
obj = sub.plot.selection.estimate(region="Canada")
maxdate=obj$date
selectionEstimateFits[["Canada"]] <- obj$fit
```
...
#selectionEstFit_BC <- (sub.plot.selection.estimate(region="British Columbia",maxdate=maxdate))$fit
...
```{r selection estimator AB, echo=FALSE, fig.height=5, fig.show="hold", fig.width=5, message=FALSE, warning=FALSE, out.width="50%"}
...
plotCaseCountSelection <- function(caseCountData, selectionEstimateObject, filename=NA){
  ...
  #selectionEstimateObject<- selectionEstFit_AB
  #filename="test"
  #caseFit <- (getCaseCountSmoothFit(caseCountData, 5))$y
  caseFit <- CubicSplSmooth(caseCountData)
  ...
  caseSelectionLines[[1]] <- list("line"=(data.frame(caseCountData$Reported_Date, caseFit)), "names" = "CaseCount", "color"="limegreen")

  #build line for each variant
  for (i in seq(from=2, to=ncol(selectionScurves))){
      x = caseCountData$Reported_Date
      actual = caseFit[1:length(selectionScurves[,i])] * selectionScurves[,i]
      proj = caseFit[(length(selectionScurves[,i])+1):length(caseFit)] * selectionScurvesExtended[,i][(length(selectionScurves[,i])+1):length(caseFit)]
      y = c(actual, proj)
  ...
  x <- caseCountData$Reported_Date
  actual = caseFit[1:length(selectionScurves[,1])] * selectionScurves[,1]
  proj = caseFit[(length(selectionScurves[,i])+1):length(caseFit)] * selectionScurvesExtended[,1][(length(selectionScurves[,1])+1):length(caseFit)]
  y = c(actual, proj)
  line <- data.frame(x,y)
  caseSelectionLines[[ncol(selectionScurves) + 1]] <- list("line"=line, "names" = "The Rest", "color"="black")

  plotCaseCountByDate2(caseCountData, rev(caseSelectionLines),numActualDatapoints=nrow(selectionScurves),filename=filename)
}
```

Some description text here...

```{r, warning=FALSE, message=FALSE, fig.height=3, fig.width=5, warning=FALSE, out.width="100%", results='asis'}
dataAvailable.regions <- all.regions %>% filter(shortname %in% c("AB", "ON", "QC"))
apply(dataAvailable.regions,1,function(reg){
  cat("####", reg[["shortname"]], "\n")
  n_min=20
  if(reg[["name"]]=="Canada"){n_min=50}
  cat("#####",reg[["name"]],"(n>",n_min,")\n","\n")
  p<-plotCaseCountSelection(caseData[[reg[["shortname"]]]], selectionEstimateFits[[reg[["shortname"]]]])
  print(p)
  cat("\n\n")})
...
recombTTree <- read.tree("./data_needed/sample4.timetree.nwk")
...
recombTimeTreeJsonObj <-  DrawTree(recombTTree, mmetasub_recomb, "timetree", presetColors, fieldnames)
#diversity ML tree
