...
#startpar <- list(p=c(0.5, 0.1), s=c(0.1, 0.1))
...
plotCaseCountSelection <- function(caseCountData, selectionEstimateObject, filename=NA){
  ...
  #caseFit <- CubicSplSmooth(caseCountData, 5)$fit
  caseFit <- 10^(getCaseCountSmoothFitWithLambda(caseCountData)$y)
...
recombTimeTreeJsonObj <-  DrawTree(recombTTree, mmetasub_recomb, "recombtimetree", presetColors, fieldnames)
#diversity ML tree
...
fit1 <- fit.rtt("./data_needed/aligned_nonrecombinant_sample1.rtt.nwk", plot=TRUE)

```


```{r}
fit2 <- fit.rtt("data_needed/aligned_nonrecombinant_sample2.rtt.nwk", plot=FALSE)
fit3 <- fit.rtt("data_needed/aligned_nonrecombinant_sample3.rtt.nwk", plot=FALSE)
```
