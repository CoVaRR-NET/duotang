...
lineageDescriptions <- read.table(paste0(params$datadir,"lineageNotes.tsv"), sep = "\t", header=T, as.is=TRUE, quote = "", stringsAsFactors = F) %>% 
  ...
  mutate(parentAlias = str_extract(alias, ".*(?=\\.)")) 

lineageDescriptions <- lineageDescriptions %>% 
  left_join((lineageDescriptions %>% select(Lineage, alias)), by=c("parentAlias"="alias"), na_matches = "never") %>%
  mutate(Parent = ifelse(is.na(Lineage.y), parentAlias, Lineage.y)) %>% 
  select(Lineage.x, Parent, Description)

colnames(lineageDescriptions) <- c("Lineage", "Ancestor", "Description")

print_datatable <- function(data) {
  # Find the row index containing the string "this is it"
  idx <- grep("B.1.1.529.5.3.1.1.1.1.1.1.5.1", data$Description)
  
  # Add a new column with hyperlinks that link to the row containing the string "this is it"
  data$link <- sprintf('<a href="#row_%s">link to row %s</a>', idx, idx)
  
  # Print the datatable
  datatable(data, escape = FALSE, 
            options = list(
              columnDefs = list(list(targets = 3, searchable = FALSE, orderable = FALSE)),
              paging = FALSE,
              info = FALSE
            ),
            rownames = FALSE,
...
#print_datatable(lineageDescriptions)
...
dataAvailable.regions <- all.regions %>% filter(shortname %in% c("BC", "AB", "SK", "MA", "ON", "QC", "East"))
for (i in seq(1:nrow(dataAvailable.regions))){
  tryCatch(
    expr = {
      selectionEstimateFits[[dataAvailable.regions[i,2]]] = sub.plot.selection.estimate(region=dataAvailable.regions[i,1],maxdate=maxdate)
    },
    error = function(e){
      selectionEstimateFits[[dataAvailable.regions[i,2]]]$plot1 <- NULL
    }
  )  
}
