...
epi <- epidataCANall[epidataCANall$prname=='Canada', ]
...
png("~/slides/img/duotang-barplot2.png", width=6.5*300, height=5*300, res=300)
{
  par(mar=c(5,5,1,5))
  pal2 <- pal[match(levels(meta1$pango.group), names(pal))]
...
This is what we want to display, but I want to be able to switch between offsets, like [this example](https://r-graph-gallery.com/155-interactive-streamgraph-change-offset.html).  I also want to be able to toggle between provinces. 
```{r}
# devtools::install_github("hrbrmstr/streamgraph")
require(streamgraph)
require(htmlwidgets)
tab <- table(meta1$pango.group, meta1$week)
dat <- data.frame(unlist(tab))
names(dat) <- c('pango.group', 'week', 'count')
plt <- streamgraph(dat, key="pango.group", value="count", date='week') %>% 
  sg_fill_manual(pal[match(levels(meta1$pango.group), names(pal))])
plt
#saveWidget(plt, file="sg_silhouette.html")
```



It looks like we'll have to write our own custom JSs to get the interactivity we want:
```{r}

tab <- table(meta1$week, meta1$pango.group)
dat1 <- as.data.frame.matrix(tab)
dat1$week <- row.names(tab)
#names(dat1) <- 

library(r2d3)
#r2d3(data=c(0.3, 0.6, 0.8, 0.95, 0.40, 0.20), script = "barchart.js")
r2d3(data=dat1, script = "streamgraph.js", viewer="external")
```
...
plot_metadatadf_freq <- function(meta_tab,cases) {
  ...
  ggplot(dfprop, aes(x=as.Date(week)))+
    ...
    scale_x_date(date_breaks = "28 day")+
    #geom_line(data=cases, aes(x=as.Date(Category),y=x),size=2,color=colorcases,alpha=0.6)+
    scale_y_continuous(name = "sequenced cases per day\naveraged by week",labels=label_number_auto())+
    theme(axis.title.y.right = element_text(color = colorcases),axis.text.y.right = element_text(color = colorcases))
}
