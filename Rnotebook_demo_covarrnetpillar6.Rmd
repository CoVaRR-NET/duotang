...
```{r message=FALSE , warning=FALSE, echo = FALSE, fig.width=5,fig.height=5, fig.show="hold", out.width="50%"}
...
for( prov in list("Alberta","British_Columbia","Ontario")){
  mydata=metaCANall %>% filter(grepl("BA.", Pango_lineage), province == "British_Columbia", !is.na(Collection_date), Collection_date >= startdate) %>% group_by(Collection_date) %>% count(Pango_lineage)
