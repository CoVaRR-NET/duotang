...
### Important Notice:  
These data analyses are based on sequence data shared by many labs across Canada and may be biased by non-random sampling, depending on local public health priorities, and should be interpreted in light of these limitations.  
...
SARS-CoV-2 viral sequence data are generated by members of the [Canadian Public Health Laboratory Network](https://nccid.ca/cphln/) and their associates. The data that they generate and the accompanying metadata (ie., sample collection date, province of origin, etc.) are shared to two public repositories: the [Canadian VirusSeq Data Portal](https://virusseq-dataportal.ca/) and [GISAID](https://www.gisaid.org/). The following graphs are meant as tools to explore data sharing across the country.
```{r data_sharing}
seqs <- read.csv('data_needed/data_reduced_mar_2022.csv', header = TRUE)
