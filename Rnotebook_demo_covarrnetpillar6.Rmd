...
Tabulation of the most predominant mutational changes in Omicron, with adjacent rows comparing the composition of Canadian sublineages to that sublineage globally ("final").
...
Mutational profile of Omicron and its sublineages in Canada and globally for the most prevalent (>75%) point mutations in each category (based on all genomes available on GISAID on the [ADD CORRECT DATE]).
...
## Evolution and growth of SARS-CoV-2 sublineages in Canada  {.tabset}  
...
**Caveat**
Selection coefficients are not estimated for Alberta, which is currently taking a variant-specific sequencing strategy, based on an initial PCR screen, which would skew estimates of selection. Canada-wide estimates, thus, do not include this province. 
...
rooted <- read.tree("./data_needed/iqtree_V1_trimmed.nwk")
vocs<-names(pal)
...
Logit plots (right panels) show $$log(p_i(t)/p_{ref}(t))$$ relative to a given reference genotype (here BA.1), which gives a line whose slope is the strength of selection $s_i$. Changes in slope indicate changes in selection on a variant (e.g., see [Otto et al.](https://mast.queensu.ca/~tday/pdf/Otto2021.pdf)). [TO ADD:  Significant changes in slope were determined by adding possible breakpoints at each week, keeping a breakpoint if it significantly increased the fit.]
