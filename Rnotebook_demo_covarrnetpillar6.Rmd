...
All analyses below draw on the most recent publicly available viral sequence data, and should be interpreted with caution due to lags in reporting and biases that can differ across provinces or territories. For example, specific variants or populations might be prioritized for sequencing at certain times in certain jurisdictions. When possible, these differences in sampling strategies are mentioned but they are not always known. These analyses are subject to change given new data.
...
Canadian genomes were obtained from the GISAID msa on the 20th of February 2022 and down-sampled to one genome per lineage, province and month + one genome of each omicron sublineage per province and month (n ~ 5500 genomes). The ML tree was reconstructed based on the GISAID alignment of these genomes using [IQ-TREE](http://www.iqtree.org/). Outliers were identified in [Tempest](http://tree.bio.ed.ac.uk/software/tempest/) and removed from the dataset. Tree time was used to estimate time tree [TreeTime](https://github.com/neherlab/treetime), and trees were plotted with [ggtree](https://github.com/YuLab-SMU/ggtree).
...
a = metaCANall %>% filter(grepl("BA.", Pango_lineage), province <= "British_Columbia", !is.na(Collection_date)) %>% group_by(Collection_date) %>% count(Pango_lineage)
b = metaCANall %>% filter(grepl("BA.", Pango_lineage), province <= "Alberta", !is.na(Collection_date)) %>% group_by(Collection_date) %>% count(Pango_lineage)
mydatalist=list(a,b)
...
for( mydata in mydatalist){
  ...
  data1 <- filter(data1, Collection_date >= startdate)
  data2 <- filter(mydata, Pango_lineage %in% name2)
  data2 <- filter(data2, Collection_date >= startdate)
  data3 <- filter(mydata, Pango_lineage %in% name3)
  data3 <- filter(data3, Collection_date >= startdate)
  
  #convert time to an integer counter for use in fitting, first using the last date as time 0:
  data1$time = as.numeric(difftime(data1$Collection_date, lastdate, units = "days"))
  data2$time = as.numeric(difftime(data2$Collection_date, lastdate, units = "days"))
  data3$time = as.numeric(difftime(data3$Collection_date, lastdate, units = "days"))
  ...
  refdate<-which(abs(toplot$n2/(toplot$n1+toplot$n2+toplot$n3)-0.5)==min(abs(toplot$n2/(toplot$n1+toplot$n2+toplot$n3)-0.5),na.rm=TRUE))
  ...
  plot(y=uppercurve1,x=toplot$date,type="l",xlab="",ylab="proportion",ylim=c(0,1))
  points(y=toplot$n2/(toplot$n1+toplot$n2+toplot$n3),x=toplot$date,pch=21, col = "black", bg = alpha("red", 0.7), cex=sqrt(toplot$n2)/3)#cex=(toplot$n2/log(10))/20)#cex=toplot$n2/50 )#cex = 0.5)
  points(y=toplot$n3/(toplot$n1+toplot$n2+toplot$n3),x=toplot$date,pch=21, col = "black", bg = alpha("blue", 0.7), cex=sqrt(toplot$n3)/3)#, cex=(toplot$n3/log(10))/20) #cex=toplot$n3/50 )#cex = 0.5)
  polygon(c(toplot$date, rev(toplot$date)), c(lowercurve1, rev(uppercurve1)),col = alpha("red", 0.5))
  polygon(c(toplot$date, rev(toplot$date)), c(lowercurve2, rev(uppercurve2)),col = alpha("blue", 0.5))
  ...
  plot(y=toplot$n2/(toplot$n1+toplot$n2+toplot$n3),x=toplot$date, pch=21, col = "black", bg = alpha(col2, 0.7), cex=sqrt(toplot$n2)/3,
       log="y",ylim=c(0.001,1000), yaxt = "n", xlab="",ylab="logit")
...
Overall, the evolutionary rate among genomes sampled in Canada (`r lm(div ~ date)$coef[2]*29970` subs/genome/day, grey line) is close to the global average of `r 0.000822*29970/365` subs/genome/day.
