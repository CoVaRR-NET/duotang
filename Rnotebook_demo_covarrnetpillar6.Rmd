...
est3$rep<-"Rep3"
est3$Lineage<-row.names(est3)
```

### Selection coefficient estimates (based on three independant subsamples)

```{r}
sec.frame<-rbind(est1,est2,est3)
sec.frame$est[sec.frame$est<0]<-0
sec.frame$lower.95[sec.frame$lower.95<0]<-0
ggplot(sec.frame,aes(x=Lineage,y=est,fill=rep))+geom_bar(position=position_dodge(),stat="identity",color="black")+
  theme(axis.text.x = element_text(size=12, angle=65,hjust=1,vjust=0.95))+  
  geom_errorbar(aes(ymin=lower.95, ymax=upper.95), width=.2,
                 position=position_dodge(1))+
  labs(y="Estimate \n (Substitutions / Genome / Day)",x="Lineage",fill="Subsample")
```
