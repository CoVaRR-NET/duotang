...
metaCANall$pango.group[grepl("B\\.1\\.1\\.7|Q\\.", metaCANall$Pango_lineage)] <- "Alpha"
metaCANall$pango.group[grepl("B\\.1\\.351", metaCANall$Pango_lineage)] <- "Beta/Lambda/Mu" # Beta
metaCANall$pango.group[grepl("P\\.", metaCANall$Pango_lineage)] <- "Gamma"
metaCANall$pango.group[grepl("B\\.1\\.617|AY\\.", metaCANall$Pango_lineage)] <- "Delta"
...
metaCANall$pango.group[grepl("C\\.37|C\\.37\\.1", metaCANall$Pango_lineage)] <- "Beta/Lambda/Mu" #lambda
metaCANall$pango.group[grepl("B\\.1\\.1\\.529|BA\\.", metaCANall$Pango_lineage)] <- "Omicron BA.1"
metaCANall$pango.group[metaCANall$Pango_lineage == "BA.1.1" ]<-"Omicron BA.1.1"
metaCANall$pango.group[metaCANall$Pango_lineage == "BA.2" ]<-"Omicron BA.2"
metaCANall$pango.group[grepl("B\\.1\\.621", metaCANall$Pango_lineage)] <- "Beta/Lambda/Mu" #Mu
...
names(pal) <- listpango
pal["Omicron BA.1"]="#8B0000"
pal["Omicron BA.1.1"]="#Fa8072"
pal["Omicron BA.2"]="#FF0000"
pal["other"]="grey"
pal["Alpha"]="#B29C71"
pal["Delta"]="#A6CEE3"
...
pal["Beta"]="#CAB2D6"
pal["Gamma"]="#444444"
pal["Beta/Lambda/Mu"]="#F08C3A"
pal["Mu"]="#F08C3A"


#------------- counts over time (by week)
mindate=as.Date("2020-12-01")
colorcases="#138808"
...
plot_metadatadf <- function(meta_tab,pal,cases) {
  ...
  cases <- cases %>% group_by(week = cut(date, "week")) #adds week coloum
  cases <- data.frame(aggregate(cases$numtoday, by=list(Category=cases$week), FUN=mean))
  cases <- cases[is.finite(cases$x),]
  ratio <- max(dfcount$n)/max(cases$x)
  cases$x=cases$x*ratio
  ggplot(dfcount, aes(x=as.Date(week)))+
    geom_bar(stat = "identity", aes(y= n, fill=pango.group))+
    scale_fill_manual(breaks=listpango, values = pal)+
    ylab("")+xlab("")+
    ...
    scale_y_continuous(name = "sequenced cases per week",sec.axis = sec_axis(~./ratio, name="recorded cases per day"))+
...
prov='Quebec'
print(epidataCANall[ which(epidataCANall$prname==prov), ])
plot_metadatadf(metaCANall[ which(metaCANall$province==prov), ],pal,epidataCANall[ which(epidataCANall$prname==prov), ])
plot_metadatadf_freq(metaCANall[ which(metaCANall$province==prov), ],pal,epidataCANall[ which(epidataCANall$prname==prov), ])
```
