plot.subvariants <- function(region='Canada', sublineage=c(name1),scaled=FALSE) {
  VOCVOI1 <- data.frame(
    name=sublineage,
    pattern=sublineage,
    color=rainbow(length(sublineage))
  )
  ...
  if (scaled) {
    ...
    legend(x=max(mp)+1, y=1, legend=levels(varmeta1$pango.group), 
