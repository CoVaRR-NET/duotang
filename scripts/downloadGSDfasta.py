...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Download the GSD FASTA")

    parser.add_argument("outfile", type=str,
                        help="output, location to save the FASTA to.")
    ...
    downloadButton = [x for x in downloadButtons if x.text == "FASTA"][0]
    downloadButton.click()
    ...
    time.sleep(3)
    while (len(glob.glob("sequences_fasta*.xz.part")) != 0):
        time.sleep(10)
    shutil.move(glob.glob("sequences_fasta*.xz")[0], args.outfile)
    
    driver.quit()
    print("GSD FASTA download completed.")
