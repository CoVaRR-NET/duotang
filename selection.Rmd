...
sublineages_NonXBB<-setAll[!setAll %in% c(sublineages_EG5, sublineage_hv1, sublineages_XBBOther)] #the rest
...
# 
mutants = list(sublineages_XBBOther, sublineage_hv1 ,sublineages_NonXBB)
mutantNames = list("XBB.Other", "HV.1*","BA.2*",  "EG.5*") #
col <- c("XBB.Other"=pal["XBB"], "HV.1*" = "#00eeff","BA.2*"="#CC5500") #define custom color for XBB here because otherwise it will be black cuz recombinant.
...
plotCaseCountSelection <- function(caseCountData, selectionEstimateObject, saveToFile=F){
  ...
  caseCountData$n <- ifelse((caseCountData$n)<1, 1, caseCountData$n) #populate empty cells with 1 because we have to log10 it later
