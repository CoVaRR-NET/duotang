...
sublineage_jn1 <- sublineage_jn1[!sublineage_jn1 %in% sublineage_jn1.11.1] #XBB Other

#sublineages_BA2Other <- getStrictoSubLineages("BA.2*", meta)
sublineages_BA2Other<-setAll[!setAll %in% c(sublineages_XBB, sublineage_jn1, sublineage_jn1.11)] #the rest

reference = sublineage_jn1
...
# 
mutants = list(sublineages_XBB, sublineage_jn1.11 ,sublineages_BA2Other)
mutantNames = list("XBB", "JN.1.11*","BA.2.Other",  "JN.1*") #
col <- c("XBB"=pal["XBB"], "JN.1.11*" = "#00eeff","BA.2.Other*"="#CC5500") #define custom color for XBB here because otherwise it will be black cuz recombinant.
...
if(length(reference)+length(Reduce(union,mutants)) - length(mutantNames) != length(setAll)){
  print(length(setAll))
...
Currently, the major group of SARS-CoV-2 lineages circulating are XBB.\* variants, including HV.1\*, as well as BA.2\* variants, particularly JN.1 and other BA.2.86 descendants. Thus, at the broad scale, we are currently track the frequencies of HV.1\*, Non-HV.1* XBB descendants (XBB.Other), JN.1* and Non-JN.1* BA.2 descendants. 
...
This plot highlights single lineages that are growing fastest (e.g., JN.1). 
