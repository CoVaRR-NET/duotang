...
sublineages_XBBOther <- sublineages_XBB[!sublineages_XBB %in% c(sublineages_XBB15, sublineages_EG5)] #XBB Other


#setAll=setAll[!setAll %in% c(sublineages_BQ, sublineages_XBB)] #the rest
sublineages_NonXBB<-setAll[!setAll %in% c(sublineages_EG5, sublineages_XBB15, sublineages_XBBOther)] #the rest

reference <- sublineages_XBB15
...
# mutants = list(sublineages_XBBOther, sublineages_EG5  )
# mutantNames = list("XBB.Other",  "EG.5*", "XBB.1.5*") #
# col <- c("XBB.Other"=pal["XBB"],"EG.5*" = "#00eeff") 

# 
mutants = list(sublineages_XBBOther, sublineages_EG5,sublineages_NonXBB  )
mutantNames = list("XBB.Other",  "EG.5*", "Non-XBB", "XBB.1.5*") #
col <- c("XBB.Other"=pal["XBB"],"EG.5*" = "#00eeff","Non-XBB*"="#CC5500") #define custom color for XBB here because otherwise it will be black cuz recombinant.
...
if(length(setAll)-length(reference)-length(Reduce(union,mutants)) + length(mutants) != 0){
  stop("sublineage groups used for selection plots might contain lineages that are in multiple groups.")
}
...
Here we examine the relative rate of spread of the different sublineages of Omicron currently in Canada. Specifically, we determine if a new or emerging lineage has a selective advantage (s), and by how much, against a reference lineage (`r individualSelectionPlotReference[[1]]`) previously common in Canada (see the methods for more details about selection and how it is estimated).

Currently, the major group of Omicron lineages rising in frequency are the XBB.* group. This group was previously dominated by the XBB.1.5* lineages, which peaked in April 2023. A variety of other XBB* have been spreading since, and we now track the frequencies of XBB.1.5\*, EG.5\*,  and XBB.Other, as well as all remaining non-XXB lineages. See Fastest Growing Lineages for details on lineages showing positive selection.

Left plot: y-axis is the proportion of these sub-lineages over time. Right plot: y-axis describes the logit function, log(freq(EG.5\*, XBB.Other, or non-XBB\*)/freq(XBB.1.5\*)), which gives a straight line whose slope is the selection coefficient if selection is constant over time (see methods).
...
#remove NB cuz it doesnt have enough data
# caseData[["NB"]] <- NULL
# caseData[["NB"]] <- NULL
...
plotCaseCountSelection <- function(caseCountData, selectionEstimateObject, saveToFile=F){
     # caseCountData <- caseData$NS
     # selectionEstimateObject<- selectionEstimateFits$NS
  # # 
  filename=NA
  ...
  if (!selectionEstimateObject$region %in% c("Alberta", "Quebec")){
  ...
  if (!selectionEstimateObject$region %in% c("Alberta", "Quebec")){
    ...
    population <- (as.numeric(populationData[1,selectionEstimateObject$region]) )
  }
...
for (i in 1:length(all.regions$name)){
  ...
  individualSublineageSelectionPlots.BA2[[all.regions$shortname[[i]]]] = plotIndividualSublineageSelection(region=all.regions$name[[i]], minCount, "BA.2*", enddate)
  individualSublineageSelectionPlots.BA5[[all.regions$shortname[[i]]]] = plotIndividualSublineageSelection(region=all.regions$name[[i]], minCount, "BA.5*", enddate)
}
