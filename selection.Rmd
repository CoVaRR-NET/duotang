...
sublineages_NonXBB<-setAll[!setAll %in% c(sublineage_hv1, sublineages_XBBOther, sublineage_jn1)] #the rest

reference = sublineage_hv1
...
# 
mutants = list(sublineages_XBBOther, sublineage_jn1 ,sublineages_NonXBB)
mutantNames = list("XBB.Other", "JN.1*","BA.2.Other",  "HV.1*") #
col <- c("XBB.Other"=pal["XBB"], "JN.1*" = "#00eeff","BA.2.Other*"="#CC5500") #define custom color for XBB here because otherwise it will be black cuz recombinant.
...
#//TODO, we need to subtract the number of root lineage names without numbers but with * (e.g. XBB*, HV*) from the total count. This arises as an artefact of the getStrictoSublineages() function. Modifying the function to remove this breaks the entire selection coefficient calculation loop. This subtraction need to be done dynamically and is usually equal to length(mutants). However, it's not always the case.. 
if(length(reference)+length(Reduce(union,mutants)) -1 != length(setAll)){
  print(length(setAll))
  print(length(reference))
  print(length(Reduce(union,mutants)))
  print(length(mutants))
  combinedList <- Reduce(union,mutants)
  dupsInList <- combinedList[duplicated(combinedList)]
  view(combinedList)
  view(setAll)
...
Here we examine the relative rate of spread of the different sublineages of Omicron currently in Canada. Specifically, we determine if a new or emerging lineage has a selective advantage (s), and by how much, against a previously common reference lineage (broad scale: `r mutantNames[length(mutantNames)]`; and in the Fastest Growing Lineages section, at the fine scale, against `r individualSelectionPlotReference[[1]]`; see [methods](#appendix) for more details about selection and how it is estimated).

Currently, the major group of Omicron lineages circulating are XBB.\* variants, including EG.5* and HV.1\*, as well as BA.2\* variants, particularly BA.2.86 and descendants. At the broad scale, we now track the frequencies of EG.5.\* (excluding HV.1\*), HV.1\*, XBB.Other, and BA.2\*. 
...
dataDownload <- coefficientTable %>% mutate(sel_coeff = sel_coeff / 100)  %>% mutate(low_CI = low_CI / 100)  %>% mutate(high_CI = high_CI / 100)  %>% mutate(reference=ifelse(grepl("\\*", lineage), individualSelectionPlotReference[[1]], mutantNames[[length(mutantNames)]]))
